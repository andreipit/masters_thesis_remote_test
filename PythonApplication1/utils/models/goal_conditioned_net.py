    ...
    def forward(self, input_color_data, input_depth_data, goal_mask_data, is_volatile=False, specific_rotation=-1):
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    if self.use_cuda:
                        ...
                        rotate_mask = F.grid_sample(Variable(goal_mask_data, volatile=True).cuda(), flow_grid_before, mode='nearest', align_corners=True)
                    else:
                        ...
                        rotate_mask = F.grid_sample(Variable(goal_mask_data, volatile=True), flow_grid_before, mode='nearest', align_corners=True)
                    ...
                    output_prob.append([nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.pushnet(interm_push_feat), push_flow_grid_after, mode='nearest', align_corners=True)),
                                        nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.graspnet(interm_grasp_feat), grasp_flow_grid_after, mode='nearest', align_corners=True))])
            ...
            if self.use_cuda:
                ...
                rotate_mask = F.grid_sample(Variable(goal_mask_data, requires_grad=False).cuda(), flow_grid_before, mode='nearest', align_corners=True)
            else:
                ...
                rotate_mask = F.grid_sample(Variable(goal_mask_data, requires_grad=False), flow_grid_before, mode='nearest', align_corners=True)
            ...
            # Forward pass through branches, undo rotation on output predictions, upsample results
            self.output_prob.append([nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.pushnet(interm_push_feat), push_flow_grid_after, mode='nearest', align_corners=True)),
                                     nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.graspnet(interm_grasp_feat), grasp_flow_grid_after, mode='nearest', align_corners=True))])
